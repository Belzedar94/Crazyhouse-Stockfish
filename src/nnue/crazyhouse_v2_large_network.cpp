...
Byte squared_activation(std::int32_t value, unsigned shift) noexcept {
    ...
    return static_cast<Byte>(std::min<std::int64_t>(
      127, squared >> static_cast<unsigned>(2 * shift + SquaredExtraShift)));
}
...
LargeNetworkLoadError LargeNetworkV1::validate_dense_intervals() const noexcept {
    for (std::size_t bucket = 0; bucket < LargeLayerStacks; ++bucket)
        for (std::size_t output = 0; output < LargeFc0Outputs; ++output)
        {
            ...
            const std::size_t base =
              (bucket * LargeFc0Outputs + output) * LargeFc0Inputs;
    ...
    for (std::size_t bucket = 0; bucket < LargeLayerStacks; ++bucket)
        for (std::size_t output = 0; output < LargeFc1Outputs; ++output)
        {
            ...
            std::int64_t upper = lower;
            const std::size_t base =
              (bucket * LargeFc1Outputs + output) * LargeFc1Inputs;
...
LargeNetworkLoadResultV1 load_large_network_v1(
  const Byte* bytes, std::size_t size, const LargeExpectedProvenanceV1& expected) noexcept {
    for (const Digest* digest : {&expected.datasetManifest, &expected.splitManifest,
                                 &expected.trainingConfig, &expected.trainerCode,
                                 &expected.trainingRuntime, &expected.resumeLineage})
    ...
    if (get_le<std::uint32_t>(bytes + 16) != ByteOrderMarker)
        return load_failure(LargeNetworkLoadError::BYTE_ORDER);
    ...
    constexpr std::array<std::uint16_t, 8> Types = {
      SignedInt16, SignedInt16, SignedInt16, SignedInt16,
      SignedInt8, SignedInt32, SignedInt32, PairProductActivation};
...
LargeNetworkV1::evaluate(const LargeFeatureInventoryV1::Result& features,
    ...
    if (features.totalPocketUnits > 30)
        return evaluation_failure(LargeNetworkEvaluateError::POCKET_UNITS);
    ...
    for (unsigned perspective = 0; perspective < COLOR_NB; ++perspective)
    {
        ...
        if (const auto error = validate_domain(featurePerspective.k64,
                                               LargeFeatureInventoryV1::KDimensions);
            error != LargeNetworkEvaluateError::NONE)
            return evaluation_failure(error);
        if (const auto error = validate_domain(featurePerspective.g1,
                                               LargeFeatureInventoryV1::GDimensions);
            error != LargeNetworkEvaluateError::NONE)
            return evaluation_failure(error);
        if (pocket_rows(featurePerspective.k64, LargeFeatureInventoryV1::KPocketOffset,
            ...
            || pocket_rows(featurePerspective.g1, LargeFeatureInventoryV1::GPocketOffset,
                 ...
                 != features.totalPocketUnits)
            return evaluation_failure(LargeNetworkEvaluateError::POCKET_ROUTING_MISMATCH);
    }

    LargeNetworkEvaluationResultV1 result;
    auto& trace = result.trace;
    trace.bucket = std::min<std::size_t>(PocketBucketMaximum,
                                         features.totalPocketUnits / PocketBucketDivisor);
    ...
    for (unsigned perspective = 0; perspective < COLOR_NB; ++perspective)
    {
        ...
        for (std::size_t lane = 0; lane < LargeKTransformerLanes; ++lane)
        {
            ...
            trace.kAccumulator[perspective][lane] = static_cast<std::int32_t>(value);
        }
        for (std::size_t lane = 0; lane < LargeGTransformerLanes; ++lane)
        {
            ...
            if (!fits_int32(value))
                return evaluation_failure(LargeNetworkEvaluateError::TRANSFORMER_RUNTIME_RANGE);
            trace.gAccumulator[perspective][lane] = static_cast<std::int32_t>(value);
        }
        trace.perspectiveOutput[perspective] =
          transform_large_pair_product_v1(trace.kAccumulator[perspective],
                                          trace.gAccumulator[perspective]);
    }
    ...
    for (std::size_t output = 0; output < LargeFc0Outputs; ++output)
    {
        ...
        const std::size_t base =
          (trace.bucket * LargeFc0Outputs + output) * LargeFc0Inputs;
        for (std::size_t input = 0; input < LargeFc0Inputs; ++input)
    ...
    for (std::size_t output = 0; output < LargeFc1Outputs; ++output)
    {
        ...
        const std::size_t base =
          (trace.bucket * LargeFc1Outputs + output) * LargeFc1Inputs;
        for (std::size_t input = 0; input < LargeFc1Inputs; ++input)
    ...
    const std::int64_t fwd = fc2 + std::int64_t(trace.fc0[LargeFc0Outputs - 2])
                           - trace.fc0[LargeFc0Outputs - 1];
    if (!fits_int32(fwd))
        return evaluation_failure(LargeNetworkEvaluateError::FC2_RUNTIME_RANGE);
